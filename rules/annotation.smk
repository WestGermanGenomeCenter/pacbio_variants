...
rule snpsift: # snps
    ...
    shell:
        ...
        """
# new versions: only one road of annotation, no single-tools annotations anymore. one after another

rule vep:
    input: # input is already annotated by snpsift, 
        vcf_phased_longp="{output_dir}/annotated_variants/snpsift_longphase_{sample}/{sample}_snpsift_longphase_annotated.vcf",
        phased_vcf_whatsh="{output_dir}/annotated_variants/snpsift_whatshap_{sample}/{sample}_snpsift_whatshap_annotated.vcf",
        vcf_nano="{output_dir}/annotated_variants/snpsift_nanocaller_{sample}/{sample}_snpsift_nanocaller_annotated.vcf"
    output:
        ...
        nanoc_snp="{output_dir}/annotated_variants/vep_nanocaller_{sample}/{sample}_vep_nanocaller_annotated.vcf"
    conda:
        "../envs/vep.yaml"
    log:
    ...
    shell:
        ...
        vep --offline --dir_cache {params.cache_dir} -i {input.vcf_phased_longp} -o {output.longp_snp}  --vcf  --everything --force_overwrite --fork {resources.threads} --warning_file {log} >>{log} 2>&1
        vep --offline --dir_cache {params.cache_dir} -i {params.whatsh_unzp} -o {output.whatsh_snp} --vcf  --everything --force_overwrite --fork {resources.threads} --warning_file {log} >>{log} 2>&1
